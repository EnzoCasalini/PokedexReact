...
const PokemonDetailsPage = () => {
	...
	const renderMoves = (movesList) => {
		...
		return (
			<Grid container columns={2} alignItems="center" justifyContent="center" sx={{marginTop: "8px"}}>
				{
					selectedMoves.map((move, index) => {
						return (
							<Grid key={index}>							
								<Chip label={move.toString().charAt(0).toUpperCase() + move.toString().slice(1)} sx={{
		...
		return (
			<Dialog onClose={onDismiss} open={true} PaperProps={{ sx: { width: "30%", height: "50%", justifyContent: "space-evenly", alignItems: "center" }}}>
				...
				<DialogContent sx={{ display: "flex", flexDirection: "column", justifyContent: "space-evenly", alignItems: "center" }}>
					...
					<Box>
						<Typography sx={{textDecoration: "underline", textAlign: "center"}}>Weight :</Typography> &nbsp; {(pokemon.weight / 10).toFixed(1)}kg
					</Box>
					<Box sx={{textAlign: "center"}}>
					<Typography sx={{textDecoration: "underline", textAlign: "center"}}>Moves :</Typography>
					{
